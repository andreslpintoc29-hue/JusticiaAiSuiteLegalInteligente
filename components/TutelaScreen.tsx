...
const TutelaScreen: React.FC = () => {
  ...
  const handleRunAnalysis = async () => {
    if (!text.trim()) return alert("El campo de texto está vacío. Ingrese datos o adjunte un PDF.");
    setIsAnalyzing(true);
    try {
    ...
    reader.onload = async (event) => {
      ...
      if (base64Data) {
        try {
          ...
          alert("Error al extraer texto del PDF. Intente pegar el texto manualmente.");
        } finally {
          setIsExtracting(false);
        }
      }
            ...
            <div className="bg-[#1e242b] rounded-xl border border-[#293038] overflow-hidden shadow-2xl">
              ...
              <div className="p-5 bg-[#161b21] border-t border-[#293038]">
                ...
                >
                  ...
                  {isAnalyzing ? 'Generando Providencia APA...' : 'Redactar Auto de Trámite'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            {result ? (
                ...
                <div className="bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-200 flex flex-col h-full">
                  <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-600 text-[10px] font-black uppercase tracking-[0.2em]">Borrador Judicial (Normas APA)</span>
                    </div>
                    <button className="bg-primary text-white text-[10px] font-black px-3 py-1 rounded hover:bg-blue-700">DESCARGAR</button>
                  </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-[#293038] rounded-2xl text-gray-700 p-12 bg-[#111418]/50">
                <span className="material-symbols-outlined text-8xl opacity-10 mb-6">edit_document</span>
                <p className="text-center font-black uppercase text-[10px] tracking-[0.4em] opacity-30">Esperando procesamiento de hechos</p>
              </div>
            )}
          </div>
        </div>
